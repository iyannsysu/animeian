...
export default function OngoingInfinite({
  initialItems,
  initialPage = 1,
}: {
  initialItems: Item[];
  ...
  const seen = useRef<Set<string>>(
    new Set(initialItems.map((i) => i.url))
  );
  ...
  const loadMore = useCallback(async () => {
    ...
    const next = page + 1;
    try {
      const res = await fetch(`/api/ongoing?page=${next}`, {
        cache: "no-store",
      });
      if (!res.ok) {
        setDone(true);
      ...
      if (fresh.length === 0 || j.hasMore === false) {
        setDone(true);
      }
      if (fresh.length > 0) {
